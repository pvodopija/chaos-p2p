...
public class MultipleServentStarter {
	...
	private static void startServentTest( String testName )
	{
		...
		ProcessBuilder bsBuilder = new ProcessBuilder(
				"java", "-cp", "bin/", "app.BootstrapServer",
				String.valueOf( AppConfig.bootstrapInfo.getListenerPort()
		) );
...
//			}
		...
		for( Process process : serventProcesses ) {
			try {
				process.waitFor(); /* Wait for graceful shutdown. */
			} catch ( InterruptedException e ) {
				e.printStackTrace();
			}
		}
		// System.out.println( "Servants destroyed." );
